defmodule Blitzy.Worker do
  ...
  when code >= 200 and code <= 304 do
    Logger.info("worker [#{worker_name()}] completed in #{msecs} msecs")
    {:ok, msecs}
  end

  defp handle_response({_msecs, {:error, reason}}) do
    Logger.info "worker [#{worker_name()}] error due to #{inspect(reason)}"
    {:error, reason}
  end

  defp handle_response({_msecs, _}) do
    Logger.info "worker [#{worker_name()}] errored out"
    {:error, :unknown}
  end

  defp worker_name, do: "#{node()}-#{inspect(self())}"
